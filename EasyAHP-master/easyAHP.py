...
#from qgis.PyQt.QtWidgets import QTableWidgetItem, QMessageBox, QTableWidgetItem, QDialog, QDialogButtonBox
...
class easyAHP(object):
    ...
    def cancel(self):
        ...
        reply = QMessageBox.question(None, 'Easy AHP', quit_msg, QMessageBox.Yes, QMessageBox.No)
        ...
        if self.dlgStep2.tableWidget.currentItem() == None:
            ...
            if self.isNumeric(self.dlgStep2.tableWidget.currentItem().text()): #checking the cell is numeric or not
                ...
                if float(self.dlgStep2.tableWidget.currentItem().text()) > 9 or float(self.dlgStep2.tableWidget.currentItem().text()) < 0.111:
                    QMessageBox.information(None,'Easy AHP', 'Your value cannot be greater than 9 and less than 0.1111')
                ...
                QMessageBox.information(None,'Easy AHP', 'Please enter a numeric value.')
                self.dlgStep2.tableWidget.currentItem().setText('')
                self.dlgStep2.tableWidget.setItem(self.dlgStep2.tableWidget.currentItem().column(), self.dlgStep2.tableWidget.currentItem().row(), QtGui.QTableWidgetItem(''))
    ...
    def browse(self):
        fileLocation, __, __ = QFileDialog.getSaveFileName(None, "Save Output File", self.dlgStep3.lineEdit.text(), "GeoTIFF file (*.tif)")
        if fileLocation:
    ...
    def saveTable(self):
        ...
        if not self.istableEdited:
            saveDlg, __, __ = QFileDialog.getSaveFileName(None, 'Save Table', '.', 'CSV file (*.csv)')
    ...
    def loadTable(self):
        ...
        loadDlg, __, __ = QFileDialog.getOpenFileName(None, 'Load Table', '.', 'CSV file (*.csv)')
    ...
    def run(self):
        ...
        #self.allMapLayers = list(qgis.core.QgsMapLayerRegistry.instance().mapLayers().items())
        self.allMapLayers = list(qgis.core.QgsProject.instance().mapLayers().items())
